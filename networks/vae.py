...
class wide_basic(nn.Module):
    def __init__(self, in_planes, planes, dropout_rate, stride=1, momentum=0.1):
...
class Wide_ResNet(nn.Module):
    def __init__(self, depth, widen_factor, dropout_rate, num_classes, momentum):
        ...
        self.layer1 = self._wide_layer(wide_basic, nStages[1], n, dropout_rate, stride=1, momentum=momentum)
        self.layer2 = self._wide_layer(wide_basic, nStages[2], n, dropout_rate, stride=2, momentum=momentum)
        self.layer3 = self._wide_layer(wide_basic, nStages[3], n, dropout_rate, stride=2, momentum=momentum)
        self.bn1 = nn.BatchNorm2d(nStages[3], momentum=momentum)
        self.linear = nn.Linear(nStages[3], num_classes)
        self.normalize = CIFARNORMALIZE(32)
        self.innormalize = CIFARINNORMALIZE(32)
    def _wide_layer(self, block, planes, num_blocks, dropout_rate, stride, momentum):
        strides = [stride] + [1]*(int(num_blocks)-1)
        layers = []

        for stride in strides:
            layers.append(block(self.in_planes, planes, dropout_rate, stride, momentum))
