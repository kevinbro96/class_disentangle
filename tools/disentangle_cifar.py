...
def train(args, epoch, model, vae, optimizer, trainloader):
    ...
    for batch_idx, (inputs, labels) in enumerate(trainloader):
        ...
        out = torch.cat( (normalize(xi), normalize(inputs) - normalize(xi)) )
        out1 = out[0:bs]
        out2 = out[bs:]
        ...
        if (batch_idx + 1) % 30 == 0:
            sys.stdout.write('\r')
            sys.stdout.write(
                '| Epoch [%3d/%3d] Iter[%3d/%3d]\t\tLoss: %.4f Loss_rec: %.4f Loss_ce: %.4f Loss_entropy: %.4f Loss_kl: %.4f Orig_Acc@1: %.3f '
                % (epoch, args.epochs, batch_idx + 1,
                   len(trainloader), loss_avg.avg, loss_rec.avg, loss_ce.avg, loss_entropy.avg, loss_kl.avg, top1.avg))

def save_model(args, epoch, model, vae):
...
def main(args):
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):
        ...
        if epoch % 10 == 1:
            ...
            model.eval()
            vae.eval()
            evaluation_class_dependent.evaluate_against_attacks(model, vae, [NoAttack()], testloader, wandb = wandb )
            reconst_images(epoch=epoch, batch_size=64, batch_num=2, dataloader=testloader, model=vae)
            save_model(args, epoch, model, vae)
