...
def train(args, epoch, model, vae, optimizer, trainloader, attack):
    ...
    for batch_idx, (inputs, labels) in enumerate(trainloader):
        ...
        out = model(normalize(all_inputs) - normalize(xi))

        l2 = F.cross_entropy(out, torch.cat((labels, labels)))
        l3 = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
        l3 /= bs * 3 * args.dim
        loss = args.ce * l2 + args.kl * l3
        loss.backward()
        optimizer.step()

        prec1, prec5, correct, pred = accuracy(out[0:inputs.size(0)].data, labels.data, topk=(1, 5))
        prec1_adv, prec5, correct, pred = accuracy(out[inputs.size(0):].data, labels.data, topk=(1, 5))
        ...
        if (batch_idx + 1) % 30 == 0:
            sys.stdout.write('\r')
            sys.stdout.write(
                '| Epoch [%3d/%3d] Iter[%3d/%3d]\t\tLoss: %.4f  Loss_ce: %.4f  Loss_kl: %.4f Orig_Acc@1: %.3f Adv_Acc@1: %.3f%%'
                % (epoch, args.epochs, batch_idx + 1,
                   len(trainloader), loss_avg.avg, loss_ce.avg, loss_kl.avg, top1.avg, adv_top1.avg))
